...
export default class Article extends Component {
  ...
  render() {
    ...
    return ( <
      ...
      icon = "minus-circle"
      onClick = {
        () => this.remove(this.state.selectedRowkKeys)
      } >
      删除 <
      /Button> < /
      Button.Group > <
      ...
      Col span = "12" >
      <
      Input.Search enterButton onSearch = {
        this.handleSearch
      }
      /> < /
      Col > <
      ...
      destroyOnClose >
      <
      WrappedEditModal wrappedComponentRef = {
        inst => (this.editform = inst)
      }
      isCreate = {
        this.state.isCreate
      }
      ...
      destroyOnClose >
      <
      WrappedCommentModal wrappedComponentRef = {
        inst => (this.commentForm = inst)
      }
      item = {
        this.state.item
      }
        ...
        Form.Item > {
          getFieldDecorator('category', {
            initialValue: this.props.isCreate ?
              this.props.categories[0]._id : this.props.item._id,
            rules: [{
              required: true,
              message: '请输入标题'
            }]
